...
@pytest.mark.asyncio
async def test_proxy_routes_require_bearer_token(monkeypatch):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.get("/v1/opencode/sessions")
        assert resp.status_code == 401

        resp = await client.get("/v1/opencode/sessions/s-1/messages")
        assert resp.status_code == 401


@pytest.mark.asyncio
async def test_proxy_routes_return_json(monkeypatch):
    import opencode_a2a.app as app_module
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        resp = await client.get("/v1/opencode/sessions", headers=headers)
        assert resp.status_code == 200
        assert resp.json()["items"][0]["id"] == "s-1"

        resp = await client.get("/v1/opencode/sessions/s-1/messages", headers=headers)
        assert resp.status_code == 200
        assert resp.json()["items"][0]["text"] == "SECRET_HISTORY"


@pytest.mark.asyncio
async def test_proxy_routes_do_not_log_response_bodies(monkeypatch, caplog):
    import opencode_a2a.app as app_module
    ...
    # The response contains SECRET_HISTORY but the log middleware must not print bodies for /v1/opencode/*
